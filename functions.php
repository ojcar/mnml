...
function mnml_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'mnml' ),
		)
	);
...
function mnml_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'mnml_content_width', 640 );
}
...
function mnml_widgets_init() {
	register_sidebar(
		array(
			...
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'mnml' ),
...
function mnml_scripts() {
	wp_enqueue_style( 'mnml-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'mnml-style', 'rtl', 'replace' );

	wp_enqueue_script( 'mnml-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
